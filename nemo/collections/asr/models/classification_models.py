...
class _EncDecBaseModel(ASRModel, ExportableEncDecModel):
    ...
    def forward(self, input_signal, input_signal_length):
        ...
        if self.fs is not None:
            processed_signal_original = processed_signal.clone()
            processed_signal_normalized = processed_signal
            processed_signal_normalized = (processed_signal_normalized - processed_signal_normalized.mean(dim=-1).unsqueeze(-1)) / torch.clamp(processed_signal_normalized.std(dim=-1).unsqueeze(-1), min=1e-8)
            noise = torch.randn_like(processed_signal, device=processed_signal.device)
            ...
            processed_signal = processed_signal_normalized * stochastic_gate

        # encoded, encoded_len = self.encoder(audio_signal=processed_signal[:,:32,:], length=processed_signal_len)
        encoded, encoded_len = self.encoder(audio_signal=processed_signal, length=processed_signal_len)
        logits = self.decoder(encoder_output=encoded)
        if self.fs is not None:
            return logits, processed_signal_original, processed_signal, fs_out
        else:
            return logits, processed_signal, processed_signal, None
...
class EncDecClassificationModel(_EncDecBaseModel):
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        audio_signal, audio_signal_len, labels, labels_len = batch
        logits, features, gated_features, _ = self.forward(input_signal=audio_signal, input_signal_length=audio_signal_len)
        if batch_idx == 0:
            for i, (original, gated, label) in enumerate(zip(features, gated_features, labels)):
                plt.clf()
                original = original.detach().cpu().numpy()
                plt.imshow(original, cmap='hot', interpolation='nearest')
                plt.gca().invert_yaxis()
                os.makedirs('outputs', exist_ok=True)
                plt.savefig(f'outputs/features_original_epoch_{self.current_epoch}_sample_{i}_label_{label.item()}.png')
                plt.clf()
                gated = gated.detach().cpu().numpy()
                plt.imshow(gated, cmap='hot', interpolation='nearest')
                plt.gca().invert_yaxis()
                plt.savefig(f'outputs/features_gated_epoch_{self.current_epoch}_sample_{i}_label_{label.item()}.png')
                if i == 20: break
